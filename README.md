...
- **`/usage`** — quota usage; ถ้าใส่ `CLAUDE_SESSION_KEY` _(optional)_ ดึง % ทางการจาก claude.ai: **5h** (%, reset at, เหลืออีกเท่าไหร่) + **7d** (%, reset at) พร้อม emoji สีตาม severity (🟢<50 🟡50‑79 🔴≥80). ไม่ใส่ key → token 5 ชม. จาก transcript (เหมือน ccusage)
...
POLL_TIMEOUT=50
# optional — เฉพาะ /usage quota % ทางการ (เว้นว่างได้)
CLAUDE_SESSION_KEY=
```
> `.env` อยู่ใน `.gitignore` แล้ว — จะไม่ถูก commit
>
> **`CLAUDE_SESSION_KEY` เป็น optional** — ไม่ใส่ก็ใช้ได้ทุกอย่าง แค่ `/usage` จะโชว์
> token จาก transcript (เหมือน ccusage) แทน quota % ทางการ. ถ้าอยากได้ % ทางการ:
> claude.ai → DevTools → Application → Cookies → ก็อป `sessionKey` (`sk-ant-sid…`) มาใส่
...
| `/usage` หรือ 📈 Usage | quota: ใส่ `CLAUDE_SESSION_KEY` → 5h/7d % + reset + severity 🟢🟡🔴 ทางการ; ไม่ใส่ → token 5 ชม. จาก transcript |
