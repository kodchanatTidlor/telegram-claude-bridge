...
- **`/status` dashboard** — ดู model, session ที่ active, prompt ค้าง + ปุ่ม refresh / สลับ session / **เปิด Claude session ใหม่** / **reload bridge**
- **`/cancel`** — หยุด Claude กลางคัน (ส่ง ESC)
- **`/usage`** — quota % ของ **ทุก account** ที่ [`cswap`](https://github.com/) จัดการ: **5h** + **7d** (%, reset at, เหลืออีกเท่าไหร่) พร้อม emoji สีตาม severity (⚪️<5 🟢<50 🟡<70 🟠<90 🔴≥90). account ที่ active มี ✅. กดปุ่ม account เพื่อ **switch** → bot จะ **reload Claude ทุก session ที่รันอยู่** (resume session เดิม) ให้ใช้ account ใหม่ แล้วบอกว่าแต่ละ session resume บน account อะไร. **ต้องติดตั้ง [`cswap`](https://github.com/) ก่อน** (requirement)
- **`/screen`** — แคปจอ session ปัจจุบันมาดูใน Telegram (เห็น TUI panel ของ Claude เช่น `/context` ที่ mirror ไม่ได้) — ลบเองเมื่อพิมพ์ข้อความถัดไป
- **♻️ Reload** — restart bridge จาก dashboard (re-exec โหลดโค้ดใหม่ ไม่ต้อง Ctrl+C เอง)
...
- **Group/Topics mode** _(optional)_ — รันหลาย session พร้อมกันแล้วตาลาย? ตั้ง `GROUP_CHAT_ID` เป็น private supergroup → แต่ละ project (cwd) แยกเป็น **forum topic** (ห้องของตัวเอง). ไม่ตั้ง = DM เหมือนเดิม
...
>
> **`/usage` ต้องมี [`cswap`](https://github.com/)** (Multi-Account Switcher for
> Claude Code) ติดตั้งไว้ — โชว์ 5h/7d % ของทุก account ที่ cswap จัดการ โดยไม่ต้อง
> ตั้งค่าอะไร. ไม่มี `cswap` → `/usage` จะบอกให้ติดตั้งก่อน
...
มี reply keyboard ถาวรใต้ช่องพิมพ์: **`[📊 Status] [📈 Usage] [📸 Screen] [🛑 Stop]`**

| คำสั่ง / ปุ่ม | ทำอะไร |
|--------------|--------|
| `/status` หรือ 📊 Status | dashboard: model, active session, prompt ค้าง + ปุ่ม refresh / reload / สลับ session / เปิด session ใหม่ |
| `/usage` หรือ 📈 Usage | quota ทุก account จาก `cswap` (**required**): 5h/7d % + reset + severity ⚪️🟢🟡🟠🔴 (active มี ✅); กดปุ่ม account = switch + reload/resume ทุก session ที่รันอยู่ |
| `/screen` หรือ 📸 Screen | แคปจอ session ปัจจุบันมาดู (code block) — ลบเองเมื่อพิมพ์ข้อความถัดไป |
...
| ♻️ Reload (ใน dashboard) | restart bridge (re-exec โหลดโค้ดใหม่) — โค้ดเท่านั้น, dep เปลี่ยนต้อง `update` ก่อน |
| 📁 (ใน dashboard) | สลับ active session |
| ➕ New (ใน dashboard) | เปิดหน้าต่าง iTerm ใหม่ + รัน Claude ที่ cwd ที่เลือก |

---

## Group / Topics mode (optional)

รันหลาย session พร้อมกันใน DM แล้วข้อความปนกัน → ย้ายไป **private supergroup**
แต่ละ project (cwd) อยู่ **forum topic** แยกห้อง

**setup ครั้งเดียว:**
1. สร้าง supergroup ส่วนตัว (มีแค่คุณ + bot) → เปิด **Topics** (group settings → Topics)
2. เพิ่ม bot เป็น **admin** ให้สิทธิ์ **Manage Topics**
3. **ปิด bot privacy** — BotFather → `/setprivacy` → เลือก bot → **Disable**
   (ไม่งั้น bot เห็นแค่ command ไม่เห็นข้อความธรรมดาที่คุณพิมพ์ใน group)
4. หา **group id**: add **@getidsbot** เข้า group → มันพิมพ์ group id (`-100…`) ให้
   ทันที → เตะ bot ตัวนั้นออก. ใส่ `.env`: `GROUP_CHAT_ID=-100…`
   (`ALLOWED_CHAT_ID` คง user id ของคุณไว้ — ใช้ยืนยันว่าเป็นคุณ)
5. restart serve

> หา group id ด้วยบอท bridge เองก็ได้ (privacy off แล้ว): หยุด serve →
> พิมพ์ใน group → `curl -s "https://api.telegram.org/bot<TOKEN>/getUpdates"`
> หา `"chat":{"id":-100…}`. แต่ @getidsbot ง่ายกว่า

**พฤติกรรม:** แต่ละ session โพสต์ลง topic ของ cwd ตัวเอง (ชื่อ = ชื่อโฟลเดอร์,
ซ้ำตอน live พร้อมกัน → `#2`); reply **ในห้องนั้น** = คุยกับ session นั้น;
topic reuse ได้เมื่อ session เดิมปิด

**ปลอดภัย:** รับคำสั่งเฉพาะ `from.id == ALLOWED_CHAT_ID` ใน group `GROUP_CHAT_ID`
เท่านั้น — สมาชิกอื่น (ถ้าเผลอเพิ่ม) สั่งไม่ได้

> v1: dashboard/`/status`/`/usage` + permission gate ยังโพสต์ใน topic General
> (ตอบ/กดปุ่มได้ปกติ); เฉพาะ stream + activity ที่แยกเข้า topic ต่อ session
